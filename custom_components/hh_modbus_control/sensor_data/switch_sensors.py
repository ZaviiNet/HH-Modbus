from custom_components.hh_modbus_control.data.enums import InverterType
...
def get_switch_sensors(inverter_config):
    ...
    elif inverter_config.type == InverterType.GRID or inverter_config.type == InverterType.STRING:
        switch_sensors.extend(
            [
                {
                    "read_register": 3089,
    ...
    for main_entity in switch_sensors:
        register = main_entity.get("register", main_entity.get("read_register"))
        for child_entity in main_entity["entities"]:
            # Unique key format: switch_{register}_{bit_position or on_value}
