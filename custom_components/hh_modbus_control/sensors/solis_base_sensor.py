...
from custom_components.hh_modbus_control.data.enums import Category, DataType, InverterFeature, PollSpeed
...
class SolisSensorGroup:
    ...
    def __init__(self, hass, definition, controller, identification=None):
        ...
        # register_type: "input" or "holding" — defaults to address-based heuristic
        # (Solis: holding registers at 40000+; Sun-Synk: explicit "holding" needed for low addresses)
        default_type = "holding" if self.start_register >= 40000 else "input"
        self.register_type: str = definition.get("register_type", default_type)

        _LOGGER.debug(
            f"Sensor group creation. start registrar = {self.start_register}, sensor count = {self.sensors_count}, "
            f"registrar count = {self.registrar_count}, register_type = {self.register_type}"
        )
    ...
    def from_sensors(
        ...
        default_type = "holding" if inst.start_register >= 40000 else "input"
        inst.register_type = register_type if register_type is not None else default_type
